...
def generate_vStorage_overview_df(df_vDisk_filtered,df_vPartition_filtered,df_vDataStore,df_vInfo_filtered):
    ...
    VMs_not_in_vPartition_with_duplicates = pd.merge(df_vDisk_filtered[['VM ID']],df_vPartition_filtered[['VM ID']],on='VM ID', how='left', indicator=True).query("`_merge`=='left_only'").drop(columns=['_merge'], axis=1)
    VMs_not_in_vPartition_unique = VMs_not_in_vPartition_with_duplicates.drop_duplicates(subset=['VM ID'])
    vDisks_not_in_vPartition = pd.merge(VMs_not_in_vPartition_unique[['VM ID']],df_vDisk_filtered[['Powerstate', 'Capacity MiB', 'VM ID']],on='VM ID', how='inner', indicator=True).query("`_merge`=='both'").drop(columns=['_merge'], axis=1)
